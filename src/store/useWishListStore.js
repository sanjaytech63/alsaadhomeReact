...
export const useWishListStore = create((set, get) => ({
    ...
    isItemInWishlist: (product_variant_id) => {
        const { wishList } = get();
        return wishList.some((item) => item.product_variant_id?.toString() === product_variant_id?.toString());
    },
    ...
    removeWishList: async (wishlist_id) => {
        set({ loading: true });
        try {
            const response = await wishListApi.removeWishList({ wishlist_id });
            if (response.status === 200) {
                set((state) => ({
                    wishList: state.wishList.filter((item) => item.wishlist_id !== wishlist_id),
                    loading: false,
                }));
                showToast("success", response.message, "success");
            }
