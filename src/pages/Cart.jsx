...
const Cart = ({ image, title, list_price, colors, sizes, pattern_image, deleteCartItem, cartItemId, itmeslug, product_id, variant_id, cart_quantity, quantity, incrementQuantity, decrementQuantity }) => {
        ...
        <CardContent sx={{ lineHeight: '2' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: "5px" }}>
              <Typography onClick={() => decrementQuantity(variant_id, quantity, cart_quantity)} sx={{ backgroundColor: "#eee", mr: 1, cursor: "pointer" }}><Remove /></Typography>
              <Typography sx={{ border: "solid 1px #ddd", px: 2 }} variant="body1"> {cart_quantity}</Typography>
              <Typography onClick={() => incrementQuantity(variant_id, quantity, cart_quantity)} sx={{ backgroundColor: "#eee", ml: 1, cursor: "pointer" }}><Add /></Typography>
            </Box>
            <Box component="div" sx={{ bgcolor: '#bb1f2a',  px: {sm:1,xs:0.5}, cursor: 'pointer', mr: { sm: 2, xs: 0 }, color: '#fff', borderRadius: '4px', ":hover": { bgcolor: 'red' } }}>
              <RiDeleteBin5Line
...
const CartPage = () => {
  ...
  const { cartItems, getCart, deleteCartItem, incrementQuantity, decrementQuantity } =
    useCartStore();
        ...
        <Container>
          <Box sx={{ display: { sm: "flex", xs: "block" }, justifyContent: "space-between", alignItems: "center", py: { sm: "30px", xs: "15px" }, fontFamily: "Roboto" }}>
            ...
            <Breadcrumbs sx={{ cursor: "pointer", fontSize: "14px" }} separator={<NavigateNextIcon fontSize="small" />} aria-label="breadcrumb">
              <Link className='breadcrumbs-hover' style={{ color: '#292b2c', textDecoration: 'none', textTransform: 'capitalize' }} to="/">
                Home
              </Link>
              {pathnames.map((segment, index) => {
                ...
                return isLast ? (
                  ...
                  <Link className='breadcrumbs-hover' key={index} style={{ color: '#292b2c', textDecoration: "none", textTransform: "capitalize" }} to={path}>
                    {decodeURIComponent(segment)}
                  </Link>
                );
              })}
            ...
            {cartItems?.branch && cartItems?.branch?.length > 0 ?
              (
                <Box sx={{ p: 2, boxShadow: " 0 0 7px rgb(0 0 0 / 10%)", mb: 1 }}>
                  <Typography sx={{ mb: 1, color: "#687188", textTransform: "capitalize", fontSize: { sm: "14px", xs: "12px" }, textAlign: "center" }}>
              ...
              (
                <Box sx={{ p: 2, boxShadow: " 0 0 7px rgb(0 0 0 / 10%)" }}>
                  <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-around", my: 2 }}>
                    <Typography variant="h5" sx={{ color: "#292b2c", textTransform: "capitalize", fontWeight: "600", fontSize: { sm: "18px", xs: "16px" } }}>
                      Sub Total
                    </Typography>
                    <Typography variant="h5" sx={{ color: "#292b2c", textTransform: "capitalize", fontWeight: "600", fontSize: { sm: "18px", xs: "16px" } }}>
                  ...
                  <Button onClick={() => navigate('/chekout')} variant="contained" sx={{ backgroundColor: "#bb1f2a", padding: "10px", mt: 1 }} fullWidth>
                    Proceed To Checkout
                  </Button>
                </Box>) : (null)
            }
