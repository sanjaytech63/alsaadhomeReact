...
const Cart = ({ image, title, list_price, colors, sizes, pattern_image, deleteCartItem, cartItemId, itmeslug, product_id, variant_id, cart_quantity, quantity }) => {
    ...
    const incrementQuantity = () => {
        if (inQut.cart_quantity < inQut.quantity) {
            setQut((prev) => ({ ...prev, cart_quantity: prev.cart_quantity + 1 }));
        } else {
            showToast("error", "Only " + inQut.quantity + "  quantity available for this product.");
        }
    };

    const decrementQuantity = () => {
        if (inQut.cart_quantity > 1) {
            setQut((prev) => ({ ...prev, cart_quantity: prev.cart_quantity - 1 }));
        }
    };
                ...
                <CardContent sx={{ lineHeight: '2' }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: "5px" }}>
                            <Typography onClick={decrementQuantity} sx={{ backgroundColor: "#eee", mr: 1, cursor: "pointer" }}><Remove /></Typography>
                            <Typography sx={{ border: "solid 1px #ddd", px: 2 }} variant="body1"> {inQut.cart_quantity}</Typography>
                            <Typography onClick={incrementQuantity} sx={{ backgroundColor: "#eee", ml: 1, cursor: "pointer" }}><Add /></Typography>
                        </Box>
                        <Box component="div" sx={{ bgcolor: '#bb1f2a',px:1, cursor: 'pointer', mr: 2 ,color: '#fff',borderRadius: '4px',":hover":{ bgcolor: 'red'}}}>
                            <RiDeleteBin5Line
                                onClick={() => deleteCartItem(cartId, cartItemId)}
